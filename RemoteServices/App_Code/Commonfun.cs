    ...
    public class Commonfun
    {
        ...
        public string Postauthxml_OnAUA(string authxml, string HttpWebRequesturl)
        {
            try
            {
                ...
                HttpWebRequest req = (HttpWebRequest)HttpWebRequest.Create(HttpWebRequesturl);
                req.Method = "POST";
            ...
            {
                var pageContent = ex.Message;
                XmlDocument xyz = new XmlDocument();
                XmlNode docxyz = xyz.CreateXmlDeclaration("1.0", "UTF-8", "yes");
                xyz.AppendChild(docxyz);
                XmlNode rootxyz = xyz.AppendChild(xyz.CreateElement("Error"));
                rootxyz.InnerXml = pageContent;
                return xyz.InnerXml;
            }
        ...
        public string ParseRespXML(string ResponseXML)
        {
            ...
            if (ret_Y == null)
            {
                ...
                ResRet = "OTP validation service failed temporarily. Please retry in some time";
                output = ResRet;
            }
            else
            {
        ...
        public string demographicParseRespXML(string ResponseXML)
        {
            ...
            {
                //ResRet = "Some Major Problem Occured";
                ResRet = "Validation service failed temporarily. Please retry in some time";
                output = ResRet;
            }
            else
            {
